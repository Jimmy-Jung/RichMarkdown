...
///     .richMarkdownCodeBlocks(.init(highlighter: PrismHighlighter.shared))
/// ```
///
/// 실패와 미지원 언어는 던지지 않고 **빈 배열**이다 — 호출한 렌더러가 원문을 그대로 둔다.
public actor PrismHighlighter: RichMarkdownSyntaxHighlighting {
    ...
    public func spans(for code: String, language: String) async -> [RichMarkdownHighlightSpan] {
    ...
    private func validatedSpans(
        ...
        source: String
    ) throws -> [RichMarkdownHighlightSpan] {
        var reconstructed = ""
        reconstructed.reserveCapacity(source.count)
        var offset = 0
        var spans: [RichMarkdownHighlightSpan] = []
        ...
        for chunk in chunks {
            ...
            if length > 0, let kind = Self.kind(for: type) {
                ...
                spans.append(RichMarkdownHighlightSpan(range: range, kind: kind))
            }
    ...
    /// 강조 대비가 사라진다.
    static func kind(for prismType: String) -> RichMarkdownHighlightKind? {
