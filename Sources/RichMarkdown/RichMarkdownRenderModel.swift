...
@MainActor
package final class RichMarkdownRenderModel: ObservableObject {
    ...
    ///
    /// `RichMarkdownRenderModel`은 `@MainActor`이고 global actor 표시는 **static 멤버에도 적용된다**.
    /// 그래서 `nonisolated` 없이 두면 이 함수 전체가 MainActor에서 실행되고
    /// `RichMarkdownParser.parse`가 main thread를 점유한다(50 KiB에서 p50 약 119ms).
    ...
    /// 이 표시가 빠졌다는 신호다.
    private nonisolated static func process(_ job: Job, model: RichMarkdownRenderModel?) async {
        guard let model else { return }
        await run(job, model: model)
        await model.markCompleted(job.generation)
    }

    private nonisolated static func run(_ job: Job, model: RichMarkdownRenderModel) async {
        ...
        if let cached = ParseCache.shared.document(for: cacheKey) {
            ...
            parsed = RichMarkdownParser.parse(
