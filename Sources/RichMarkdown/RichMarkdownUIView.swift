import Combine
import UIKit
// RichMarkdownTheme의 SwiftUI.Color를 UIColor로 변환하기 위해서만 쓴다. 뷰 계층은 순수 UIKit이다.
import SwiftUI
import RichMarkdownCore

/// UIKit 네이티브 렌더러. SwiftUI 호스팅 래퍼가 아니다.
///
/// 파싱(`RichMarkdownCore`), 수식 raster(`MathRenderService`), generation 관리
/// (`RichMarkdownRenderModel`)는 `RichMarkdownView`와 동일한 경로를 공유하고
/// 이 타입은 뷰 계층만 담당한다.
///
/// ```swift
...
/// ```
public final class RichMarkdownUIView: UIView {
    ...
    public var codeBlocks: RichMarkdownCodeBlockOptions = .none {
        didSet {
    ...
    public var streaming: RichMarkdownStreamingOptions? {
        didSet {
    ...
    let model = RichMarkdownRenderModel()
    /// 테스트에서 블록 구성 검증에 쓴다.
    ...
    private struct RenderedBlock {
        ...
        let tail: RichMarkdownStreamingOptions?
    }
    ...
    private struct AppearanceKey: Equatable {
        ...
        let codeBlocks: RichMarkdownCodeBlockOptions
    }
    ...
    private lazy var fallbackTextView: RichMarkdownTextView = textView(NSAttributedString())

    /// 테스트의 조건 기반 idle 대기용 상태다. 외부 API 계약은 아니다.
    var hasPendingRebuild: Bool { rebuildScheduled || contentSizeChangeScheduled }

    public init(markdown: String = "", parsesDollarMath: Bool = false, theme: RichMarkdownTheme = .default) {
    ...
    public init(markdown: String = "", dollarMath: LatexDollarMathOptions, theme: RichMarkdownTheme = .default) {
        self.boundedMarkdown = InputLimits.bound(markdown)
    ...
    required init?(coder: NSCoder) {
        fatalError("RichMarkdownUIView는 코드로만 생성한다")
    }
    ...
    private var currentRequest: RichMarkdownRenderModel.Request {
    ...
    private func rebuild() {
        let signpostState = RichMarkdownSignposts.rebuild.beginInterval("rebuild")
        defer { RichMarkdownSignposts.rebuild.endInterval("rebuild", signpostState) }
    ...
    private func updateBlockInPlace(
        ...
        case (.paragraph, .paragraph(let runs)):
            guard let textView = view as? RichMarkdownTextView else { return false }
    ...
    private func updateCodeBlock(_ view: UIView, language: String?, code: String) -> Bool {
        guard codeBlocks.diagramRenderer(for: language) == nil,
              ...
              let copy = header.arrangedSubviews.last as? RichMarkdownCopyButton,
              let scroll = container.arrangedSubviews[1] as? HorizontalScrollBlock,
              let body = scroll.content as? RichMarkdownTextView
        else { return false }
    ...
    private func blockView(
        _ block: ParsedBlock,
        images: [MathSegment: RenderedMath],
        tail: RichMarkdownStreamingOptions? = nil
    ) -> UIView {
        switch block {
    ...
    private func applyHighlight(to view: RichMarkdownTextView, code: String, language: String?) {
        ...
        view.highlightTask = Task { @MainActor [weak view] in
            ...
            view.attributedText = RichMarkdownHighlightSegments.attributed(
                code: code,
                spans: spans,
                colors: colors,
                font: font,
    ...
    private func textView(_ attributed: NSAttributedString) -> RichMarkdownTextView {
    ...
    private func copyButton(text: String, accessibilityLabel: String) -> UIButton {
        let button = RichMarkdownCopyButton(type: .system)
        button.payload = text
...
final class RichMarkdownCopyButton: UIButton {
    var payload = ""
}
...
final class TableScrollBlock: HorizontalScrollBlock {
    var cells: [[RichMarkdownTextView]] = []
