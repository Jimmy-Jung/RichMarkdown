...
public enum RichMarkdownTextStyle: String, Sendable, Hashable, CaseIterable {
    case body
    case headline
    case title1
    case title2
    ...
    /// `RichMarkdownFont.size`가 nil일 때의 기준값이고, 수식 raster의 기준 크기이기도 하다.
...
public enum RichMarkdownFontWeight: String, Sendable, Hashable, CaseIterable {
    case ultraLight
    case thin
    case light
    case regular
...
/// 요소별 폰트 지정.
///
/// `Sendable` 값 타입이라 `RichMarkdownTheme`에 담고 수식 렌더 요청 key에도 넣을 수 있다.
...
/// ```
public struct RichMarkdownFont: Sendable, Hashable {
    ...
    /// Dynamic Type 스케일 기준.
    public var relativeTo: RichMarkdownTextStyle
    /// nil 또는 0 이하/비유한 값이면 `relativeTo`의 기본 크기를 쓴다.
    public var size: CGFloat?
    /// nil이면 스타일의 기본 굵기를 유지한다. `.custom` 서체에서는 무시될 수 있다.
    public var weight: RichMarkdownFontWeight?

    public init(
        design: Design = .standard,
        relativeTo: RichMarkdownTextStyle = .body,
        size: CGFloat? = nil,
        weight: RichMarkdownFontWeight? = nil
    ) {
        self.design = design
    ...
    /// `RichMarkdownFont`가 `Set`/cache key에서 Hashable 계약을 깨게 된다.
    public static func == (lhs: RichMarkdownFont, rhs: RichMarkdownFont) -> Bool {
    ...
    /// 유효한 명시 point size만 반환한다. 공개 `RichMarkdownTheme` 입력은 앱 설정이나 원격
    ...
    /// `RichMarkdownBlockEditor`처럼 `RichMarkdownTheme`의 폰트를 UIKit attributed string으로
    /// 옮기는 외부 렌더러를 위해 공개한다.
    ...
    /// system/monospaced font는 `RichMarkdownFontScale`을 통해 이미 계산된 size를
...
/// 환경값은 `RichMarkdownView`가 style별로 측정해 이 값으로 전달한다. 공개 theme
/// API에 새 설정을 추가하지 않기 위해 module 내부에만 둔다.
struct RichMarkdownFontScale: Sendable, Hashable {
    var body: CGFloat = 1
    ...
    func factor(for style: RichMarkdownTextStyle) -> CGFloat {
        let factor: CGFloat
        switch style {
...
private struct RichMarkdownFontScaleKey: EnvironmentKey {
    static let defaultValue = RichMarkdownFontScale()
}

extension EnvironmentValues {
    var richMarkdownFontScale: RichMarkdownFontScale {
        get { self[RichMarkdownFontScaleKey.self] }
        set { self[RichMarkdownFontScaleKey.self] = newValue }
    }
}

private struct RichMarkdownFontModifier: ViewModifier {
    let font: RichMarkdownFont
    @Environment(\.richMarkdownFontScale) private var scale
...
extension View {
    /// 명시 point size도 root의 `RichMarkdownFontScale`에 따라 Dynamic Type으로 확대한다.
    func richMarkdownFont(_ font: RichMarkdownFont) -> some View {
        modifier(RichMarkdownFontModifier(font: font))
    }
}
