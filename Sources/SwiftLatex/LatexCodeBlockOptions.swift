...
///
/// 코어는 프로토콜만 선언하고 구현은 별도 product가 담당한다. `SwiftLatex`만 쓰는 앱은
/// JavaScriptCore도 WebKit도, 3 MB 넘는 JavaScript 번들도 링크하지 않는다.
///
/// ```swift
/// LatexMarkdownView(markdown: message)
///     .latexCodeBlocks(.init(highlighter: PrismHighlighter.shared, diagram: MermaidDiagramRenderer.shared))
/// ```
public struct LatexCodeBlockOptions: Sendable, Equatable {
    /// 코드 블록 본문에 색 역할을 부여한다. `nil`이면 지금까지처럼 plain monospace다.
    public var highlighter: (any LatexSyntaxHighlighting)?
    /// 담당 언어의 코드 블록을 다이어그램 뷰로 대체한다.
    public var diagram: (any LatexDiagramRendering)?

    public init(
        highlighter: (any LatexSyntaxHighlighting)? = nil,
        diagram: (any LatexDiagramRendering)? = nil
    ) {
    ...
    /// 아무 확장도 걸지 않은 기본값.
    public static let none = LatexCodeBlockOptions()

    /// 이 코드 블록 언어를 담당하는 다이어그램 렌더러. 없으면 `nil`이다.
    func diagramRenderer(for language: String?) -> (any LatexDiagramRendering)? {
    ...
    public static func == (lhs: LatexCodeBlockOptions, rhs: LatexCodeBlockOptions) -> Bool {
...
public enum LatexHighlightKind: String, Sendable, Hashable, CaseIterable {
    case keyword
    case string
    case comment
    case number
...
public struct LatexHighlightSpan: Sendable, Hashable {
    public let range: NSRange
    public let kind: LatexHighlightKind

    public init(range: NSRange, kind: LatexHighlightKind) {
        self.range = range
        self.kind = kind
    }
}

/// 코드 블록 하이라이터. 구현은 `SwiftLatexHighlight`의 `PrismHighlighter`다.
///
/// 동기 호출이 아니다: 토큰화는 렌더러의 MainActor 밖에서 끝내고, 도착한 범위만
/// 색으로 반영한다. 미지원 언어와 실패는 **빈 배열**이며 오류를 던지지 않는다.
public protocol LatexSyntaxHighlighting: AnyObject, Sendable {
    func spans(for code: String, language: String) async -> [LatexHighlightSpan]
}

// MARK: - 다이어그램

/// 특정 언어의 코드 블록을 통째로 대체하는 뷰 공급자.
/// 구현은 `SwiftLatexMermaid`의 `MermaidDiagramRenderer`다.
public protocol LatexDiagramRendering: AnyObject, Sendable {
    ...
    /// SwiftUI 렌더러용 뷰.
    @MainActor
    func makeSwiftUIView(source: String, theme: LatexTheme) -> AnyView
}

// MARK: - 주입

private struct LatexCodeBlockOptionsKey: EnvironmentKey {
    static let defaultValue = LatexCodeBlockOptions.none
}

extension EnvironmentValues {
    var latexCodeBlocks: LatexCodeBlockOptions {
        get { self[LatexCodeBlockOptionsKey.self] }
        set { self[LatexCodeBlockOptionsKey.self] = newValue }
    }
}

public extension View {
    /// 코드 블록 하이라이터·다이어그램 렌더러를 주입한다.
    func latexCodeBlocks(_ options: LatexCodeBlockOptions) -> some View {
        environment(\.latexCodeBlocks, options)
    }
}
...
enum LatexHighlightSegments {
    struct Segment {
        let text: Substring
        let kind: LatexHighlightKind?
    }

    static func segments(code: String, spans: [LatexHighlightSpan]) -> [Segment] {
