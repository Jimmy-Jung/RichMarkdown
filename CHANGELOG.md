...
- **Mermaid 다이어그램이 가로 중앙에 놓인다.** 컨테이너보다 좁은 다이어그램이 왼쪽 끝에
  붙던 문제를 고쳤다. `useMaxWidth: false`로 확대하지 않는 정책 때문에 세로형 플로우차트는
  가용 폭보다 좁아지는데, 호스트 페이지의 `#diagram svg`에 중앙 정렬이 없어 남는 폭이 전부
  오른쪽으로 몰렸다 (`margin-inline: auto` 추가). 넓은 화면(iPad, 분할 뷰)에서 두드러졌다.
  - 컨테이너보다 넓은 다이어그램은 이전과 같이 가용 폭에 맞춰 축소되며 여백이 없다.
  - `index.html` SHA-256이 바뀌었다 (`Docs/CODE_BLOCK_EXTENSIONS.md` §5 갱신).
