...
@Suite struct StreamingTailTests {
    ...
    private func runs(_ markdown: String, dollar: Bool = false) -> [InlineRun] {
        let document = RichMarkdownParser.parse(markdown: markdown, parsesDollarMath: dollar)
    ...
    @Test func doubleDollarOpenerIsHiddenOnlyWithInlineDoubleOption() {
        func hiddenText(_ markdown: String, _ options: DollarMathOptions) -> String {
            let document = RichMarkdownParser.parse(markdown: markdown, dollarMath: options)
