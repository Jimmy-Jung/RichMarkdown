...
@Suite struct MathScannerFixtureTests {
    ...
    private func parse(_ markdown: String, dollar: Bool = false) -> ParsedDocument {
        RichMarkdownParser.parse(markdown: markdown, parsesDollarMath: dollar)
    }

    private func parse(_ markdown: String, dollarMath: DollarMathOptions) -> ParsedDocument {
        RichMarkdownParser.parse(markdown: markdown, dollarMath: dollarMath)
    }
    ...
    @Test func decodedEntityCannotCollideWithOpaqueMathMarker() {
        let doc = parse(#"&#xE000;richmarkdown-0&#xE001; &amp; \(x\)"#)
        #expect(
            plainText(of: firstParagraphRuns(doc))
                == "\u{E000}richmarkdown-0\u{E001} & \\(x\\)"
        )
        #expect(doc.allMathSegments.map(\.latex) == ["x"])
    }
