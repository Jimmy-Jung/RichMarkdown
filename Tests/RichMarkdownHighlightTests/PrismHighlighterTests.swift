...
@testable import RichMarkdownHighlight
...
@Suite struct PrismHighlighterTests {
    ...
    private func kinds(_ spans: [RichMarkdownHighlightSpan]) -> Set<RichMarkdownHighlightKind> {
        Set(spans.map(\.kind))
    }
    ...
    @Test func jsonSeparatesPropertyFromString() async {
        let code = #"{"name": "RichMarkdown", "version": 7, "beta": true}"#
        ...
        // `"name"`은 property, `"RichMarkdown"`는 string이어야 한다 — 실제 문자로 확인한다.
