...
@Suite struct ThemeFontTests {
    ...
    private func makeView(_ markdown: String, theme: RichMarkdownTheme) async throws -> RichMarkdownUIView {
        let view = RichMarkdownUIView(markdown: markdown, theme: theme)
    ...
    // MARK: - RichMarkdownFont 해석 단위 검증

    @Test func customFontFamilyResolves() {
        let font = RichMarkdownFont(design: .custom(name: "Georgia"), relativeTo: .body, size: 20)
    ...
    @Test func unknownCustomFontFallsBackToSystem() {
        let font = RichMarkdownFont(design: .custom(name: "존재하지않는서체XYZ"), relativeTo: .body, size: 20)
        let resolved = font.resolvedUIFont(compatibleWith: nil)
    ...
    @Test func weightAppliesToStandardDesign() {
        let regular = RichMarkdownFont(relativeTo: .body).resolvedUIFont(compatibleWith: nil)
        let heavy = RichMarkdownFont(relativeTo: .body, weight: .heavy).resolvedUIFont(compatibleWith: nil)
    ...
    @Test func monospacedDesignResolvesToFixedWidthFont() {
        let font = RichMarkdownFont(design: .monospaced, relativeTo: .body).resolvedUIFont(compatibleWith: nil)
    ...
    @Test func invalidExplicitFontSizeFallsBackToStyleDefault() {
        for invalidSize in [CGFloat(-1), 0, .nan, .infinity] {
            let font = RichMarkdownFont(relativeTo: .title2, size: invalidSize)
            #expect(font.unscaledSize == RichMarkdownTextStyle.title2.defaultSize)
        }
    }

    @Test func invalidExplicitFontSizesHaveStableHashableIdentity() {
        let defaultFont = RichMarkdownFont(relativeTo: .title2)
        for invalidSize in [CGFloat(-1), 0, .nan, .infinity] {
            let invalidFont = RichMarkdownFont(relativeTo: .title2, size: invalidSize)
    ...
    @Test func headingStylesUseMatchingUIKitAndSwiftUILevels() {
        #expect(RichMarkdownTextStyle.title1.uiTextStyle == .title1)
        #expect(RichMarkdownTextStyle.title1.swiftUITextStyle == .title)
        #expect(RichMarkdownTextStyle.title2.uiTextStyle == .title2)
        #expect(RichMarkdownTextStyle.title2.swiftUITextStyle == .title2)
        #expect(RichMarkdownTextStyle.title3.uiTextStyle == .title3)
        #expect(RichMarkdownTextStyle.title3.swiftUITextStyle == .title3)
    }

    private func renderedTextImage(font: RichMarkdownFont, scale: RichMarkdownFontScale) throws -> UIImage {
        let renderer = ImageRenderer(
            content: Text("RichMarkdown")
                .richMarkdownFont(font)
                .fixedSize()
                .environment(\.richMarkdownFontScale, scale)
        )
        renderer.scale = 1
    ...
    @Test func explicitSwiftUISystemSizeUsesDynamicTypeScale() throws {
        let font = RichMarkdownFont(relativeTo: .title2, size: 24)
        var enlarged = RichMarkdownFontScale()
        enlarged.title2 = 1.8

        let normalImage = try renderedTextImage(font: font, scale: RichMarkdownFontScale())
    ...
    @Test func bodyFontSizeAppliesToParagraph() async throws {
        let theme = RichMarkdownTheme(bodyFont: RichMarkdownFont(relativeTo: .body, size: 40))
    ...
    @Test func headingFontOverrideApplies() async throws {
        let theme = RichMarkdownTheme(heading1Font: RichMarkdownFont(relativeTo: .title1, size: 50, weight: .black))
    ...
    @Test func codeFontOverrideAppliesToInlineAndBlock() async throws {
        let theme = RichMarkdownTheme(codeFont: RichMarkdownFont(design: .monospaced, relativeTo: .body, size: 30))
    ...
    @Test func codeLabelFontOverrideApplies() async throws {
        let theme = RichMarkdownTheme(codeLabelFont: RichMarkdownFont(design: .monospaced, relativeTo: .caption, size: 28))
    ...
    @Test func listMarkerFollowsBodyFont() async throws {
        let theme = RichMarkdownTheme(bodyFont: RichMarkdownFont(relativeTo: .body, size: 33))
    ...
    @Test func mathRasterSizeFollowsBodyFont() async throws {
        let small = try await makeView(
            #"인라인 \(a+b\) 수식"#,
            theme: RichMarkdownTheme(bodyFont: RichMarkdownFont(relativeTo: .body, size: 17))
        )
        let large = try await makeView(
            #"인라인 \(a+b\) 수식"#,
            theme: RichMarkdownTheme(bodyFont: RichMarkdownFont(relativeTo: .body, size: 40))
        )
