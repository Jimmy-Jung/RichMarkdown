...
@Suite struct ThemeReachTests {
    ...
    private static var markerTheme: RichMarkdownTheme {
        RichMarkdownTheme(textColor: marker)
    }
    ...
    private func blockView(_ block: ParsedBlock) -> some View {
        RichMarkdownBlockView(block: block, images: [:])
            .richMarkdownTheme(Self.markerTheme)
    }
    ...
    @Test func swiftUIInlineCodeUsesInlineCodeForeground() throws {
        ...
        let view = RichMarkdownBlockView(block: block, images: [:])
            .richMarkdownTheme(RichMarkdownTheme(inlineCodeForeground: Self.marker))
        #expect(containsMarker(try render(view)))
    }

    @Test func swiftUIRawFallbackUsesThemeTextColor() throws {
        let view = RichMarkdownView(markdown: "파싱 전 원문").richMarkdownTheme(Self.markerTheme)
        #expect(containsMarker(try render(view)))
    }
    ...
    @Test func defaultThemeDoesNotProduceMarkerPixels() throws {
        let block = ParsedBlock.paragraph([InlineRun(content: .text("본문 글자"))])
        let view = RichMarkdownBlockView(block: block, images: [:]).richMarkdownTheme(.default)
        #expect(!containsMarker(try render(view)))
    }
    ...
    private func inkCount(theme: RichMarkdownTheme, bold: Bool = false) throws -> Int {
        let view = RichMarkdownBlockView(block: paragraph("가나다 ABC", bold: bold), images: [:])
            .richMarkdownTheme(theme)
        return markerPixelCount(try render(view))
    }

    @Test func swiftUIBodyFontSizeChangesRendering() throws {
        let small = try inkCount(theme: RichMarkdownTheme(textColor: Self.marker, bodyFont: RichMarkdownFont(relativeTo: .body, size: 17)))
        let large = try inkCount(theme: RichMarkdownTheme(textColor: Self.marker, bodyFont: RichMarkdownFont(relativeTo: .body, size: 40)))
        #expect(large > small, "bodyFont 크기가 커지면 잉크 픽셀이 늘어야 한다")
    }

    @Test func swiftUICustomFontFamilyChangesRendering() throws {
        let system = try inkCount(theme: RichMarkdownTheme(textColor: Self.marker, bodyFont: RichMarkdownFont(relativeTo: .body, size: 28)))
        let georgia = try inkCount(theme: RichMarkdownTheme(
            textColor: Self.marker,
            bodyFont: RichMarkdownFont(design: .custom(name: "Georgia"), relativeTo: .body, size: 28)
        ))
    ...
    @Test func swiftUIBoldRunRendersHeavier() throws {
        let theme = RichMarkdownTheme(textColor: Self.marker, bodyFont: RichMarkdownFont(relativeTo: .body, size: 28))
    ...
    @Test func uiKitAppliesThemeTextColorToEveryTextElement() async throws {
        ...
        let theme = RichMarkdownTheme(textColor: Self.marker, inlineCodeForeground: Self.marker)
