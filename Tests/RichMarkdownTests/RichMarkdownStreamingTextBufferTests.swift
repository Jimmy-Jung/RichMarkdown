...
@MainActor
@Suite struct RichMarkdownStreamingTextBufferTests {
    ...
    @Test func firstUpdatePublishesSynchronously() {
        let buffer = RichMarkdownStreamingTextBuffer(interval: .milliseconds(50))
        buffer.update("첫")
    ...
    @Test func updateAfterIntervalPublishesImmediately() async throws {
        let buffer = RichMarkdownStreamingTextBuffer(interval: .milliseconds(20))
        buffer.update("a")
    ...
    @Test func flushPublishesPendingSynchronously() {
        let buffer = RichMarkdownStreamingTextBuffer(interval: .seconds(10))
        buffer.update("a")
        buffer.update("ab")
    ...
    @Test func resetDropsPendingAndCancelsTrailing() async throws {
        let buffer = RichMarkdownStreamingTextBuffer(interval: .milliseconds(30))
        buffer.update("old")
