...
/// `TEST_RUNNER_RICHMARKDOWN_STREAM_SECONDS=30 xcodebuild test ...`로 실행한다.
@MainActor
@Suite(.serialized) struct StreamingBaselineTests {
    ...
    @Test func parseTimingBaseline() {
        ...
        for _ in 0..<30 {
            ...
            _ = RichMarkdownParser.parse(markdown: fixture, parsesDollarMath: false)
    ...
    ///
    /// `RichMarkdownRenderModel`은 `@MainActor`이고 global actor 표시는 static 멤버에도 적용되므로,
    ...
    @Test func tenHertzStreamingContract() async throws {
        let seconds = Double(ProcessInfo.processInfo.environment["RICHMARKDOWN_STREAM_SECONDS"] ?? "5") ?? 5
        let fixture = Self.makeFixture()
