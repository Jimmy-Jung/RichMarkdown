...
/// UIKit 네이티브 렌더러(`RichMarkdownUIView`) 확인 화면.
///
...
enum RichMarkdownThemePreset: String, CaseIterable, Identifiable {
    case standard = "기본"
    case large = "큰 글자"
    case serif = "Serif"
    case tinted = "색 강조"
    ...
    var theme: RichMarkdownTheme {
        ...
        case .large:
            return RichMarkdownTheme(
                bodyFont: RichMarkdownFont(relativeTo: .body, size: 24),
                heading1Font: RichMarkdownFont(relativeTo: .title1, size: 38, weight: .bold),
                heading2Font: RichMarkdownFont(relativeTo: .title2, size: 30, weight: .bold),
                heading3Font: RichMarkdownFont(relativeTo: .title3, size: 26, weight: .semibold),
                codeFont: RichMarkdownFont(design: .monospaced, relativeTo: .body, size: 20),
                codeLabelFont: RichMarkdownFont(design: .monospaced, relativeTo: .caption, size: 16)
            )
...
extension RichMarkdownThemePreset {
    /// UI 테스트 스크린샷 매트릭스용. `-richmarkdownPreset Serif` 형태로 넘긴다.
    /// 기존 `-richmarkdownDark`와 같은 방식이다.
    static func fromLaunchArguments() -> RichMarkdownThemePreset {
        let arguments = ProcessInfo.processInfo.arguments
        guard let index = arguments.firstIndex(of: "-richmarkdownPreset"),
              index + 1 < arguments.count,
              let preset = RichMarkdownThemePreset(rawValue: arguments[index + 1])
...
struct UIKitChatConfiguration: Equatable {
    ...
    var preset: RichMarkdownThemePreset = .standard

    /// preset 테마에 수식 정렬 주입을 합성한 최종 테마.
    var resolvedTheme: RichMarkdownTheme {
...
struct UIKitChatDemoView: View {
    @State private var configuration = UIKitChatConfiguration(
        preset: RichMarkdownThemePreset.fromLaunchArguments()
    )
