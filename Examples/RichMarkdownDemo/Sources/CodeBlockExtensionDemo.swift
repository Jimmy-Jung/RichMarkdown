...
///
/// 두 확장 모두 `RichMarkdownCodeBlockOptions` 주입으로만 켜진다. 끄면 라이브러리 기본 동작
...
struct CodeBlockExtensionDemoView: View {
    ...
    private var options: RichMarkdownCodeBlockOptions {
            ...
            ToolbarItem(placement: .topBarTrailing) {
                Menu {
                    ...
                    Picker("테마", selection: $preset) {
                        ForEach(RichMarkdownThemePreset.allCases) { value in
                            Text(verbatim: value.rawValue).tag(value)
                        }
                    }
                } label: {
    ...
    private var content: some View {
        ...
        case .swiftUI:
            ScrollView {
                RichMarkdownView(markdown: Self.sample, parsesDollarMath: true)
                    .richMarkdownTheme(preset.theme)
...
/// `RichMarkdownUIView`를 넣고 높이는 자동 레이아웃이 결정하게 둔다.
...
final class CodeBlockExtensionUIKitController: UIViewController {
    ...
    var options: RichMarkdownCodeBlockOptions {
        didSet { messageView.codeBlocks = options }
    }

    private let scrollView = UIScrollView()
    private let messageView: RichMarkdownUIView

    init(markdown: String, theme: RichMarkdownTheme, options: RichMarkdownCodeBlockOptions) {
        self.theme = theme
        self.options = options
        self.messageView = RichMarkdownUIView(markdown: markdown, parsesDollarMath: true, theme: theme)
