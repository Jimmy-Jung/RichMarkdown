...
/// SSE 스트리밍을 UIKit 네이티브 렌더러(`RichMarkdownUIView`)로 확인하는 화면.
///
...
final class UIKitSSEDemoViewController: UIViewController {
    ...
    private let buffer = RichMarkdownStreamingTextBuffer(interval: SSEDemoLimits.publishInterval)
    ...
    init(theme: RichMarkdownTheme, parsesDollarMath: Bool) {
        self.theme = theme
