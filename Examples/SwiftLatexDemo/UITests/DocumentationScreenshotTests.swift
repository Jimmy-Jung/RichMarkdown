...
///
/// ```bash
/// TEST_RUNNER_SWIFTLATEX_CAPTURE_DOCS=1 xcodebuild test \
///   -project SwiftLatexDemo.xcodeproj -scheme SwiftLatexDemo \
///   -destination 'platform=iOS Simulator,name=iPhone 16 Pro,OS=18.6' \
///   -only-testing:SwiftLatexDemoUITests/DocumentationScreenshotTests \
...
final class DocumentationScreenshotTests: XCTestCase {
    private var capturesDocumentation: Bool {
        ProcessInfo.processInfo.environment["SWIFTLATEX_CAPTURE_DOCS"] == "1"
    }
    ...
    func testCaptureReadmeScreenshots() throws {
        try XCTSkipUnless(capturesDocumentation, "문서 갱신 전용 — SWIFTLATEX_CAPTURE_DOCS=1")

        let app = XCUIApplication()
        app.launch()
        XCTAssertTrue(app.navigationBars["SwiftLatex Demo"].waitForExistence(timeout: 30))
